...
def vk_callback():
    ...
    # ============================================
    # ВРЕМЕННО ОТКЛЮЧЕНО: VK ID API не поддерживает groups.get
    # ============================================
    ...
    # Пока подключаем только личную страницу автоматически
    user_groups = []  # Пустой список групп
    
    print(f"ℹ️ VK ID OAuth пока поддерживает только личные страницы")
    print(f"📝 Функция групп будет добавлена позже")
    
    # ============================================
    # АВТОМАТИЧЕСКОЕ ПОДКЛЮЧЕНИЕ ЛИЧНОЙ СТРАНИЦЫ
    # ============================================
    
    # Создаём свежее подключение к БД
    from database.database import Database
    db = Database()
    
    try:
        # Сохраняем подключение личной страницы напрямую
