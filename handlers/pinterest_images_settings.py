...
def handle_pinterest_images_menu(call):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "🔙 Назад",
            callback_data=f"back_to_pinterest_{category_id}_{bot_id}"
        )
    )
...
def handle_pinterest_preview_format_select(call):
    ...
    markup.row(
        types.InlineKeyboardButton(
            "🔙 Назад",
            callback_data=f"platform_format_pinterest_{category_id}_{bot_id}"
        )
    )
...
def handle_back_to_pinterest(call):
    """Возврат в меню Pinterest категории"""
    parts = call.data.split("_")
    category_id = int(parts[3])
    bot_id = int(parts[4])
    ...
    # Редирект на главное меню платформы Pinterest
    call.data = f"platform_menu_{category_id}_{bot_id}_pinterest_{platform_id}"
